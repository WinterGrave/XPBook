...
import zairus.xpbook.client.renderer.ISpecialRendered;
...
public class ClientProxy extends CommonProxy
{
	...
	public void registerModel(Item item, int meta, String name)
	{
		...
		ModelResourceLocation modelResourceLocation = new ModelResourceLocation(XPBConstants.MODID + ":" + name, "inventory");
		
		renderItem.getItemModelMesher().register(item, meta, modelResourceLocation);
		
		if (meta == 0)
			ModelBakery.registerItemVariants(item, new ResourceLocation(XPBConstants.MODID, name));
	}
